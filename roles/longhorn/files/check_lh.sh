...
# Looping through all of the required deployed resources during 5 minutes with 10 seconds intervals
count=0
while [ "$count" -lt 30 ]
do
    # Functions and variables to calculate the number of resource to determine the success of the deployment
    DESIRED_RESOURCE_NUMBER=0
    AVAILABLE_RESOURCE_NUMBER=0

    add_desired() {
        DESIRED_RESOURCE_NUMBER=$((DESIRED_RESOURCE_NUMBER+$1))
    }
    ...
    then
        ...
        # Generate a list of nodes that Longhorn is installed on
        ...
        for node in $LONGHORN_NODE_LIST
        do
    ...
    if [ "$DESIRED_RESOURCE_NUMBER" -eq "$AVAILABLE_RESOURCE_NUMBER" ]
    then
        echo "All resources deployed successfully"
        break
    else
        echo "Not all resources deployed yet"
    fi
